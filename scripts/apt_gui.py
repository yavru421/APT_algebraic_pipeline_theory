...
Variables:
- x1: selected_rapt_file
- x2: workspace_context
- Y: complete GUI state
"""
...
class APTGUIApp:
    def __init__(self):
        ...
        self.current_rapt_content: str = ""
        self.parsed_expression: Dict[str, Any] = {}
        self.workspace_tabs: Dict[str, Any] = {}
    ...
    def main(self, page: ft.Page):
        ...
        y1 = self.m1_file_selector()
        y2 = self.m2_expression_renderer()
        y3 = self.m3_tab_manager()
    ...
    def m1_file_selector(self) -> ft.Column:
        ...
        def on_select_current():
            # Load the current RAPT_PIPELINE.RAPT file
        ...
        return ft.Column([
            ...
            ft.Row([
                ...
                ft.ElevatedButton(
                    ...
                    on_click=lambda _: on_select_current()
                )
            ]),
    ...
    def create_workspace_tab(self, tab_name: str, file_path: str, root_dir: str):
        ...
        # Directory listing
        dir_list = ft.ListView(height=200, spacing=2)
        try:
            ...
            for item in sorted(root_path.iterdir()):
                dir_list.controls.append(
                    ft.ListTile(
                        ...
                        leading=ft.Icon(ft.icons.FOLDER if item.is_dir() else ft.icons.FILE_PRESENT),
                        on_click=lambda e, path=item: self.open_file_in_tab(path, tab_name)
                    )
                )
        except:
            dir_list.controls.append(ft.Text(f"Could not list directory: {root_dir}"))

        # Terminal output
        terminal_output = ft.Text("Terminal ready...", size=12, color=ft.colors.GREEN)
        terminal_input = ft.TextField(
            hint_text="Enter command...",
            on_submit=lambda e: self.run_terminal_command(e.control.value, terminal_output, root_dir)
        )

        # Toggle between file and terminal view
        file_view = ft.Column([
            ft.Text(f"File: {file_path}", weight=ft.FontWeight.BOLD),
            ft.Container(
        ...
        tab_content = ft.Column([
            ft.Row([
                ft.ElevatedButton("File View", on_click=lambda _: toggle_view("file")),
                ft.ElevatedButton("Terminal View", on_click=lambda _: toggle_view("terminal"))
            ]),
    ...
    def open_file_in_tab(self, file_path: Path, tab_name: str):
        """Open a file in the current tab"""
        # This would update the file view in the current tab
        pass

    def run_terminal_command(self, command: str, output_widget: ft.Text, working_dir: str):
        ...
        try:
            ...
            output_widget.color = ft.colors.GREEN if result.returncode == 0 else ft.colors.RED
            self.page.update()

        except subprocess.TimeoutExpired:
            output_widget.value = f"$ {command}\nCommand timed out"
            output_widget.color = ft.colors.ORANGE
            self.page.update()
        except Exception as e:
            output_widget.value = f"$ {command}\nError: {e}"
            output_widget.color = ft.colors.RED
            self.page.update()
